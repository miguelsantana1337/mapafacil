...
export async function signUp(data: FormData) {
  ...
  const { data: result, error } = await supabase.auth.signUp({
    ...
    password: value(data, "password"),
    options: {
      data: { full_name: value(data, "name") },
  ...
  if (error) {
    const message = error.code === "over_email_send_rate_limit"
      ? "O serviço de e-mail atingiu o limite temporário. Tente novamente em alguns minutos."
  ...
  if (result.session) redirect("/app");
...
export async function resetPassword(data: FormData) {
  const supabase = await createClient();
  const origin = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
  await supabase.auth.resetPasswordForEmail(value(data, "email"), {
